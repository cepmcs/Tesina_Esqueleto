% =========================================================================
% Figura: Representación de moléculas en SELFIES (Aspirina 2D vs. SELFIES)
...
% Flecha curva de cierre autorreferenciado en el ciclo (de C2 a C1, salto relativo Delta=5)
\draw[line width=1.2pt, -{Stealth[length=4.5pt, width=3.5pt]}, draw=figBlue]
    ($(C2)+(-0.35, 0.15)$) to[bend left=38] node[midway, left, font=\fontsize{6.5}{7.5}\selectfont\bfseries, text=figBlue] {$\Delta=5$} ($(C1)+(-0.35, -0.15)$);
...
% =========================================================================
% PANEL DERECHO: STRING SELFIES (SERIALIZACIÓN ROBUSTA)
% =========================================================================
\node[anchor=north west, font=\bfseries\small, text=darkText] at (8.1, 6.55) {Secuencia SELFIES};
\node[anchor=north west, font=\scriptsize, text=grayText] at (8.1, 6.15) {Tokens con longitud y salto autorreferenciados};

% Cápsula SELFIES completa destacada (en 2 líneas perfectamente contenidas)
\node[
    draw=cardBorder,
    fill=cardBorder!15,
    rounded corners=6pt,
    ...
    inner sep=5pt,
    anchor=north west
] (selfies_bar) at (8.1, 5.75) {
    \begin{minipage}{7.2cm}
    \raggedright\fontsize{6.3}{8.2}\selectfont\ttfamily
    \colorbox{figOrangeTint}{\textbf{\color{figOrange}[C][C][=Branch1][C][=O][O]}}%
    \colorbox{figBlueTint}{\textbf{\color{figBlue}[C][=C][C][=C]}}\\[2.5pt]
    \colorbox{figBlueTint}{\textbf{\color{figBlue}[C][=C][Ring1][=Branch1]}}%
    \colorbox{figGreenTint}{\textbf{\color{figGreen}[C][=Branch1][C][=O][O]}}
    \end{minipage}
};

% 3 Bloques visuales correspondientes a cada fragmento del grafo
% Bloque 1: Ramificación lateral con tamaño explícito (Naranja)
\node[
    draw=figOrange,
    fill=figOrangeTint,
    rounded corners=5pt,
    ...
    \begin{minipage}{7.0cm}
        {\fontsize{9.5}{11}\selectfont\ttfamily\textbf{\color{figOrange}[C][C][=Branch1][C][=O][O]}}\\[1.5pt]
        {\small\textbf{\color{darkText}Ramificación lateral} \texttt{[=Branch1]}}
    \end{minipage}
};

% Bloque 2: Estructura cíclica con Ring1 (Azul)
\node[
    draw=figBlue,
    fill=figBlueTint,
    rounded corners=5pt,
    ...
    \begin{minipage}{7.0cm}
        {\fontsize{7.5}{9}\selectfont\ttfamily\textbf{\color{figBlue}[C][=C][C][=C][C][=C][Ring1][=Branch1]}}\\[1.5pt]
        {\small\textbf{\color{darkText}Estructura cíclica} \texttt{[Ring1]} ($\Delta=5$)}
    \end{minipage}
};
...
\node[
    ...
    \begin{minipage}{7.0cm}
        {\fontsize{9.5}{11}\selectfont\ttfamily\textbf{\color{figGreen}[C][=Branch1][C][=O][O]}}\\[1.5pt]
        {\small\textbf{\color{darkText}Ramificación terminal} \texttt{[=Branch1]}}
    \end{minipage}
};

% =========================================================================
% FLECHAS DE CONEXIÓN ENTRE FRAGMENTO 2D Y BLOQUE SELFIES
% =========================================================================
...
\caption{Representación de la molécula de aspirina en notación SELFIES y correspondencia con su grafo 2D.}
\label{fig:selfies_aspirina}
\end{figure}
